...
from src.model.punc_model import BertPunc
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', '-mdl', type=str, default='mobilebert')
    parser.add_argument('--base_mdl', '-bm', type=str, default="mdl/bert-base-chinese")
    parser.add_argument('--scheduler', '-sd', type=str, default='OneCycleLR')
    params = parser.parse_args()
    model = BertPunc(params)
    ...
    trainer = Trainer(max_epochs=params.max_epochs, logger=logger, default_root_dir="./output", callbacks=[save_model], strategy='ddp_find_unused_parameters_true' )
    datamodule = PunctDataModule(batch_size=params.batch_size)
